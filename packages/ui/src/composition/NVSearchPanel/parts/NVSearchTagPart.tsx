...
export const NVSearchTagPart = ({ tags, toggleTag, isDesktop = false }: NVSearchTagPartProps) => (
  <div className={cn("flex flex-wrap", isDesktop ? "gap-2 py-1" : "gap-1.5")}>
    ...
    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-dashed border-white/10 text-[11px] font-bold text-slate-500 hover:text-slate-300 hover:border-white/20 transition-all">
      <Tag size={12} /> 전체 보기
    </button>
  </div>
);
