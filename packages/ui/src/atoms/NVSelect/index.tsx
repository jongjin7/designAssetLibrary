...
export const NVSelect: React.FC<NVSelectProps> = ({ 
  ...
  const selectClasses = `
    bg-transparent border-none text-nv-text-secondary outline-none 
  ...
  return (
    <div className={containerClasses}>
      ...
      <select className={selectClasses} {...props}>
        {options.map((option) => (
          <option key={option.value} value={option.value} className="bg-nv-surface text-nv-text-primary">
            {option.label}
          </option>
        ))}
      </select>
      <div className={`absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-nv-text-tertiary`}>
