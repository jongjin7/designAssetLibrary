...
NOVA는 4px 배수 시스템을 기반으로 한 `sp-` 유틸리티를 사용하여 일관된 레이아웃 리듬을 유지합니다.

| Token | Pixels | Usage |
| :--- | :--- | :--- |
| `sp-1` | 4px | 컴포넌트 내부의 미세 요소 간격. |
| `sp-2` | 8px | 인접한 텍스트 또는 아이콘 간격. |
| `sp-4` | 16px | 컴포넌트 내부 패딩 및 표준 요소 간격. |
| `sp-6` | 24px | 그룹화된 요소 간의 큰 간격. |
| `sp-8` | 32px | 섹션 간의 구분 또는 페이지 여백. |
