...
│ │ 추출 컬러  🔴🔵🟡🟢⚫   │ │  ← PaletteStrip
│ │                         │ │
│ │ 태그  #ui[x] #app[x] [+] │ │  ← TagList (Chip + Delete + Add)
│ │                         │ │
│ │ [공유]  [이동]  [삭제]  │ │  ← ActionBar
│ └─────────────────────────┘ │
└─────────────────────────────┘
...
*   **AI Metadata**: AI가 분석한 객체(Object), 구도(Composition), 5가지 주요 색상(Palette) 나열.
*   **Tag Management**: 
    *   **삭제**: 태그 칩(Chip) 우측의 'x' 버튼 클릭 시 즉시 제거.
    *   **추가**: 태그 리스트 끝의 '+' 버튼 클릭 시 입력 필드가 활성화되며 신규 태그 입력 가능.
